...
export default function CustomerCollection() {
    ...
    const fetchCustomerOrders = async () => {
      try {
        ...
        const response = await axios.get<ICustomerOrder[]>(
          `http://distromentor.onrender.com/customerOrder/getAllCustomerOrdersByDealerId/${userFromStorage.userId}`
        );
  ...
  const mapRows = (orders: ICustomerOrder[]) =>
    orders.map((order) => ({
      id: order.orderid,
      customerName: `${order.customer?.firstName ?? "N/A"} ${order.customer?.lastName ?? "N/A"}`,
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      width: 100,
      renderCell: (params) => (
        <StyledButton onClick={() => navigate(`/customerTransaction/${params.row.id}`)}>View</StyledButton>
      ),
    },
  ];
        ...
        <Box sx={{ width: "100%", marginTop: 4, marginLeft: 0.5 }}>
          <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
            <Tabs value={tabValue} onChange={(e, val) => toggleTables(val)} style={{ marginLeft: 40 }}>
              <TabStyle label="Unpaid Customer Order" value="Open" />
              <TabStyle label="Paid Customer Order" value="Closed" />
            </Tabs>
          </Box>
          <Box sx={{ p: 2 }}>
            {isLoading ? (
