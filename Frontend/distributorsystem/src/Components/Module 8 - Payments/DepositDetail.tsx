...
export default function DepositDetailsUI() {
  ...
  const fetchDealerById = async (dealerId: string) => {
    try {
      const dealerResponse = await axios.get<IDealer>(`http://distromentor.onrender.com/dealer/getDealerByID/${dealerId}`);
      setDealer(dealerResponse.data);

      // Fetch dealer total sales (Total Order Amount), total SRP, and profit
      const salesResponse = await axios.get<IDealerTotalResponse>(`http://distromentor.onrender.com/dealerTotals/getByDealerId/${dealerId}`);
      ...
      const debtResponse = await axios.get<IProductSubtotalResponse>(`http://distromentor.onrender.com/allProductSubtotals/getByDealerId/${dealerId}`);
  ...
  const updateDealerTotalSalesAndDebt = async () => {
    ...
    try {
      ...
      await axios.put(`http://distromentor.onrender.com/dealerTotals/updateByDealerId/${dealer!.dealerid}`, {
      ...
      await axios.put(`http://distromentor.onrender.com/allProductSubtotals/updateByDealerId/${dealer!.dealerid}`, {
    ...
    <StyledCard>
      ...
      {deposit && (
        <>
          ...
          <Typography gutterBottom>
            <a href={`http://distromentor.onrender.com${deposit.proofOfRemittance}`} target="_blank" rel="noopener noreferrer">
              View Proof
            </a>
          </Typography>
